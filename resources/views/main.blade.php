                    ...
                    <div class="flex gap-2 items-center w-max">
                        ...
                        <div class="flex gap-2 items-center h-full">
                            <div class="dropdown w-max">
                                ...
                                <button class="btn flex gap-2" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    @for($i =0; $i < sizeof($collabors); $i++) @if($i==2) @break @endif <img src="{{$collabors[$i]->profile_img !== 'unset'?asset($collabors[$i]->profile_img):asset('asset/imags/profile-img.png')}}" class="w-6 h-6 rounded-full">
                                        @endfor
                                </button>
                                @endif
                                    ...
                                    <div class="w-full h-full">
                                        ...
                                        <li class="flex mb-2 py-2 border-b gap-2 items-center collabor-row">
                                            <div class="flex w-full gap-2 items-center">
                                                <img src="{{$projectCreator->profile_img !== 'unset'?asset($projectCreator->profile_img):asset('asset/imags/profile-img.png')}}" class="w-6 h-6 rounded-full">
                                        ...
                                        <li class="flex mb-2 py-2 border-b gap-2 items-center collabor-row">
                                            <div class="flex w-full gap-2 items-center">
                                                <img src="{{$collabor->profile_img !== 'unset'?asset($collabor->profile_img):asset('asset/imags/profile-img.png')}}" class="w-6 h-6 rounded-full">
                                            ...
                                            <form action="{{route('delete.collaboration', ['project_id'=>$project->id, 'user_id'=>$collabor->user_id])}}" method="POST">
                                                @csrf
                                                <button type="submit" class=" btn btn-dark bg-black py-0 px-2" style="font-size: 0.7rem;">Kick</button>
                                            </form>
                                            @endif
                                        </li>
                                        @endforeach
                        ...
                        <div class="border border-gray-400 rounded-2xl cursor-pointer flex items-center gap-2 text-sm px-2 py-0.5 text-gray-400" id="invite-btn">
